...
class PaymentReport extends Component {
  ...
  preset(data) {
    for (let i = 0; i < data.length; i++) {
      ...
      var td1 = document.createElement("td");
      var td1input = document.createElement("input");
      td1input.id = i + "_title";
      td1input.value = data[i].title;
      td1.className = "pl-4 py-2 inputStyle flex flex-col";
      td1.appendChild(td1input);
  ...
  async printDocument() {
    ...
    var data = {
      ...
      subp: subp,
      subop: subop,
      paid_history: this.state.history,
      Remains: this.state.remains
    };

    axios.post(BASELINE + "invoice/update/by/id", data)
      .then(function (response) {
        alert(response.data.message);
      })
      .catch(function (error) {
  ...
  componentDidMount() {
    this.GetClassItem();
    this.bring_invoice(this.props?.invoice_id);
  }
                      ...
                      <div className="p-3 pb-1 flex items-center gap-2">
                        <div className="relative flex-1">
                          ...
                          <input
                            type="number"
                            name="remains"
                            value={this.state.remains}
                            className="bg-gray-50 border ml-[90px] border-gray-500 text-gray-700 text-md rounded-lg focus:ring-blue-300 focus:border-blue-300 block w-200 px-2.5 py-1 outline-none"
                            onChange={this.changetext}
                          />
                        </div>
                      </div>
                      <div className="p-3 pb-1 flex items-center gap-2">
                        <div className="relative flex-1">
                          ...
                          >
                            ...
                            {this.state.status_lst.map((el, i) =>
